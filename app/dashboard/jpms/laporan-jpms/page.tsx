...
export default function LaporanPage() {
    ...
    const fetchData = async (p = 1) => {
        ...
        try {
            ...
            const res = await fetch(
                `${API}/laporan2.php?${params.toString()}`,
                { cache: "no-store" }
            );
    ...
    const openDetail = async (id: number) => {
        try {
            const resDetail = await fetch(
                `${API}/laporan-detail2.php?id=${id}`,
                { cache: "no-store" }
            );
            const detailJson = await resDetail.json();

            const resImg = await fetch(
                `${API}/laporan-images2.php?id=${id}`,
                { cache: "no-store" }
            );
    ...
    const submitVerifikasi = async () => {
        try {
            ...
            const res = await fetch(`${API}/verifikasi-laporan2.php`, {
                method: "POST",
    ...
    const submitDelete = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API}/hapus-laporan2.php?id=${deleteId}`, {
                method: "GET",
            });
        ...
        <div className="min-h-screen bg-linear-to-br from-blue-50 via-indigo-50 to-purple-50 p-6 space-y-6">
            ...
            <div className="bg-linear-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-2xl shadow-2xl p-8 text-white">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold mb-1">Data Laporan Material 1.2 </h1>
